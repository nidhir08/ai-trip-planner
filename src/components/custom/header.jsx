...
function Header() {
  const [user, setUser] = useState(() => JSON.parse(localStorage.getItem('user'))); // initialize state from localStorage
  const [dialog, setDialog] = useState(false);
  ...
  // Sync user state with localStorage on login/logout
  useEffect(() => {
  ...
  const login = useGoogleLogin({
    onSuccess: (codeResponse) => {
      console.log(codeResponse); // Access token obtained
      GetUserProfile(codeResponse); // Call GetUserProfile with the token
    },
    onError: (error) => console.log(error),
  });
  ...
  const GetUserProfile = (tokenInfo) => {
    axios
      ...
      .then((resp) => {
        console.log("called", resp);
        localStorage.setItem('user', JSON.stringify(resp.data));
        setUser(resp.data);  // Update user state with the new user data
        setDialog(false);  // Close the dialog after successful login
      });
  };
